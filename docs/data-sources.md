...
| HYCOM | Public THREDDS / OPeNDAP (stretch: particle advection). | Public model output; distributor terms on the THREDDS node in use. | Acknowledge the HYCOM consortium and the specific run / experiment ID. | No |
| NDBC buoys | [ndbc.noaa.gov](https://www.ndbc.noaa.gov/). No API key. | NOAA open / NODD-class public data. | Same NODD rules. | No |
